...
;(function(mod) {
    ...
    CodeMirror.defineMode('sql', function(config, parserConfig) {
        ...
        function tokenBase(stream, state) {
            // let reference = config.referenceFn || (s => '#' + s);
            // let refRe = new RegExp('^' + _.escapeRegExp(reference('SPLITTER')).replace('SPLITTER', '(\\w*)'))

            if (config.refRe && stream.match(config.refRe)) {
                return 'variable-2'
            }
