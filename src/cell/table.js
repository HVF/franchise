...
export class TableVisualizer extends React.Component {
    ...
    render() {
        ...
        function renderCell(rowIndex, colIndex) {
            ...
            let changed_value = db.getStagingValue(
                deltas,
                raw_value,
                result,
                rowIndex,
                    ...
                    <Column
                        ...
                        renderColumnHeader={(colIndex) => (
                            <ColumnHeaderCell
                                ...
                                renderName={(name) => (
                                    <div className="bp-table-truncated-text">
                                        {isColumnEditable(colIndex) ? (
                                            <i className="fa-pencil fa editable-icon" />
                                        ) : null}{' '}
                                        {name}{' '}
                                    </div>
                                )}
                ...
                onConfirm={async (value, rowIndex, colIndex) => {
                    ...
                    if (value === this.props.value) {
                        ...
                        try {
                            ...
                            requestAnimationFrame((_) => swal.fire('Oops...', err.message, 'error'))
                        }
                        clearTimeout(this.cancelTimeout)
                        this.setState({ loading: false })
                    }
...
export class CardVisualizer extends React.Component {
    ...
    static icon = <i className="fa fa-vcard-o" aria-hidden="true" />
    ...
    render() {
        ...
        function renderRow(field, colIndex, rowIndex) {
            if (
                hideNull &&
                !(getStagingValue(rowIndex, colIndex) || getRawValue(rowIndex, colIndex))
            )
                return null

            return (
                <tr key={field + rowIndex}>
                    <th>
                        {field}{' '}
                        {isColumnEditable(colIndex) ? (
                            <i className="fa-pencil fa editable-icon" style={{ color: '#bbb' }} />
                        ) : null}
                    </th>
            ...
            <div className={'single-row ' + (view.loading ? 'result-loading ' : '')}>
                    ...
                    {insertable ? (
                        <div style={{ paddingLeft: 20 }}>
                            <Button
                                ...
                                onClick={(e) => this.setState({ inserting: !this.state.inserting })}
                            >
                                Insert
                            </Button>
                        </div>
                ...
                connect.schema.find((k) => k.name == result.tableName) ? (
                    <table>
                        <tbody>
                            {connect.schema
                                .find((k) => k.name == result.tableName)
                    ...
                    <table>
                        <tbody>
                            {result.columns.map((field, colIndex) =>
                                renderRow(field, colIndex, rowIndex)
                            )}
                        </tbody>
                    </table>
                )}
...
export class PivotVisualizer extends React.Component {
    ...
    static icon = <span className="pt-icon-standard pt-icon-pivot-table" />
    ...
    render() {
        ...
        if (pivotIndices.length != 2) {
            return (
                <div className="single-row">Currently only 2 axis pivot tables are supported.</div>
            )
        }

        function getPivotValue(xPivot, yPivot) {
            let match = result.values.find(
                (k) => k[pivotIndices[0].index] === yPivot && k[pivotIndices[1].index] === xPivot
            )
            if (match)
                return valIndices
                    .map((k) => {
        ...
        return (
            <div className="pivot-visualizer">
                <table className="pt-table pt-bordered">
                    <thead>
                        <tr>
                            <td />
                            {pivotIndices[1].values.map((xPivot) => (
                                <th>{xPivot}</th>
                            ))}
                        </tr>
                    </thead>
                    <tbody>
                        {pivotIndices[0].values.map((yPivot) => (
                            <tr>
                                <td>
                                    <b>{yPivot}</b>
                                </td>
                                {pivotIndices[1].values.map((xPivot) => (
                                    <td>{getPivotValue(xPivot, yPivot)}</td>
                                ))}
                            </tr>
                        ))}
                    </tbody>
